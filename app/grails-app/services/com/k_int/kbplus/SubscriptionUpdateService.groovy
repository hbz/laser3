...
class SubscriptionUpdateService extends AbstractLockableService {
    ...
    void assignNoteOwners() {
        ...
        docsWithoutOwner.each { DocContext dc ->
            ...
            if(dc.subscription) {
                if(dc.isShared) {
                    if(dc.subscription.getCalculatedType() == CalculatedType.CALCULATED_TYPE_CONSORTIAL)
                        documentOwner = dc.subscription.getConsortia()
                    else if(dc.subscription.getCalculatedType() == CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE)
