        ...
        if (params.q?.length() > 0) {
            ...
            base_qry += (
                    " and ( genfunc_filter_matcher(s.name, :name_filter) = true " // filter by subscription
                            ...
                            + " or exists ( select li from Links li where li.destination = concat('"+Subscription.class.name+":',s.id) and li.linkType = :linkType and li.source in (:licenses) ) " // filter by license
