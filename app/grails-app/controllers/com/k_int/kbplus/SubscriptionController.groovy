...
class SubscriptionController extends AbstractDebugController {
    ...
    def delete() {
        ...
        else if(result.subscription.getCalculatedType() in [CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE, CalculatedType.CALCULATED_TYPE_COLLECTIVE, CalculatedType.CALCULATED_TYPE_CONSORTIAL, CalculatedType.CALCULATED_TYPE_ADMINISTRATIVE])
    ...
    def unlinkPackage() {
        ...
        def queryParams = [sub: result.subscription, pkg_id: result.package.id]
            ...
            if (params.confirmed) {
                ...
                if(result.subscription.getCalculatedType() in [CalculatedType.CALCULATED_TYPE_CONSORTIAL, CalculatedType.CALCULATED_TYPE_COLLECTIVE, CalculatedType.CALCULATED_TYPE_ADMINISTRATIVE] &&
...
//        result.offset = params.offset ? Integer.parseInt(params.offset) : 0;
    ...
    def linkLicenseMembers() {
        ...
        result.navPrevSubscription = links.prevLink
        result.navNextSubscription = links.nextLink
        result.parentSub = result.subscriptionInstance.instanceOf && result.subscriptionInstance.getCalculatedType() != CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE ? result.subscriptionInstance.instanceOf : result.subscriptionInstance
    ...
    def processLinkLicenseMembers() {
        ...
        if(result.subscriptionInstance.getCalculatedType() == CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE)
    ...
    def linkPackagesMembers() {
        ...
        def oldID = params.id
        if(result.subscription.getCalculatedType() in [CalculatedType.CALCULATED_TYPE_CONSORTIAL, CalculatedType.CALCULATED_TYPE_COLLECTIVE])
            params.id = result.parentSub.id
    ...
    def propertiesMembers() {
        ...
        if(result.subscriptionInstance.getCalculatedType() == CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE && result.institution.id == result.subscription.getCollective().id)
    ...
    def subscriptionPropertiesMembers() {
        ...
        result.parentSub = result.subscriptionInstance.instanceOf && result.subscription.getCalculatedType() != CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE ? result.subscriptionInstance.instanceOf : result.subscriptionInstance
                ...
                members.each { cm ->
                    ...
                    //if (true) {
                        ...
                        Subscription memberSub = new Subscription(
                                ...
                                startDate: startDate,
                                endDate: endDate,
                                administrative: result.subscriptionInstance.getCalculatedType() == CalculatedType.CALCULATED_TYPE_ADMINISTRATIVE,
                        ...
                        if (memberSub) {
                            if(accessService.checkPerm("ORG_CONSORTIUM")) {
                                if(result.subscriptionInstance.getCalculatedType() == CalculatedType.CALCULATED_TYPE_ADMINISTRATIVE) {
    ...
    */
    ...
    private Map<String,Object> setResultGenericsAndCheckAccess(checkOption) {
        ...
        if(result.subscription.getCollective()?.id == contextService.getOrg().id &&
                (result.subscription.getCalculatedType() == CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE ||
    ...
    static boolean showConsortiaFunctions(Org contextOrg, Subscription subscription) {
        return ((subscription?.getConsortia()?.id == contextOrg?.id) && subscription.getCalculatedType() in
                [CalculatedType.CALCULATED_TYPE_CONSORTIAL, CalculatedType.CALCULATED_TYPE_ADMINISTRATIVE])
    }
    ...
    static boolean showCollectiveFunctions(Org contextOrg, Subscription subscription) {
        return ((subscription?.getCollective()?.id == contextOrg?.id) && subscription.getCalculatedType() in
                [CalculatedType.CALCULATED_TYPE_COLLECTIVE, CalculatedType.CALCULATED_TYPE_PARTICIPATION_AS_COLLECTIVE])
    }
