...
class SubscriptionController extends AbstractDebugController {
    ...
    def index() {
        ...
        List pendingChanges = PendingChange.executeQuery("select pc.id from PendingChange as pc where subscription=? and ( pc.status is null or pc.status = ? ) order by ts desc", [result.subscriptionInstance, pending_change_pending_status]);
    ...
    */
    ...
    def show() {
        ...
        if (executorWrapperService.hasRunningProcess(result.subscriptionInstance)) {
            ...
            def pending_change_pending_status = RefdataValue.getByValueAndCategory('Pending', RDConstants.PENDING_CHANGE_STATUS)
            List pendingChanges = PendingChange.executeQuery("select pc.id from PendingChange as pc where subscription=? and ( pc.status is null or pc.status = ? ) order by pc.ts desc", [result.subscription, pending_change_pending_status])
