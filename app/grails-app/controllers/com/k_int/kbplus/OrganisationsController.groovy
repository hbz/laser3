...
class OrganisationsController {
    ...
    def show() {
      ...
      link_vals.each { lv ->
        ...
        if(lv.id){
          def cur_param = "rdvl_${Long.parseLong(lv.id)}"
          ...
          def link_type_count = OrgRole.executeQuery("select count(*) from OrgRole as orgr where orgr.org = :oi and orgr.roleType.id = :lid",[oi: orgInstance, lid: Long.parseLong(lv.id)])
          def link_type_results = OrgRole.executeQuery("select orgr from OrgRole as orgr where orgr.org = :oi and orgr.roleType.id = :lid",[oi: orgInstance, lid: Long.parseLong(lv.id)],[offset:param_offset,max:10])
          
          if(link_type_results){
            sorted_links["${Long.parseLong(lv.id)}"] = [rdv: lv.value, rdvl: cur_param, links: link_type_results, total: link_type_count[0]]
          }
        }else{
