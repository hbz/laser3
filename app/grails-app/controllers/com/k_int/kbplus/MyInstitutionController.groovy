...
//            jusp_heads_row.setHeight((short) (jusp_heads_row.getHeight() * 2));
        ...
        if(params.tsvFile) {
            ...
            rows[0].split('\t').eachWithIndex { String headerCol, int c ->
                switch(headerCol.toLowerCase()) {
                    ...
                    case "währung":
                    case "waehrung":
                    case "currency": colMap.billingCurrency = c
                        break
                    ...
                    case "wert (in eur)":
                    case "value":
                    case "value (in euro)": colMap.costInLocalCurrency = c
                        break
                    case "lizenz":
                    ...
                    case "organisation": colMap.institution = c
                        break
                    default: log.info("unhandled parameter type, ignoring ...")
                        break
                }
            }
            rows.remove(0)
            ...
            rows.each { row ->
                ...
                if(!orgIdentifier)
                    owner = (Org) result.institution
                else {
                    //fetch possible identifier namespaces
                    List<Org> orgMatches = Org.executeQuery("select idOcc.org from IdentifierOccurrence idOcc join idOcc.identifier id where idOcc.org.id = :idCandidate or idOcc.org.globalUID = :idCandidate or (id.value = :idCandidate and id.ns = :wibid)",[idCandidate:orgIdentifier,wibid:namespaces.wibid])
                    if(!orgMatches) {
                        ...
                        List<Org> egpMatches = Org.executeQuery("select opp.owner from OrgPrivateProperty opp where opp.intValue = :idCandidate and opp.type = :egp",[idCandidate: orgIdentifier,egp: egpNr])
                        if(!egpMatches)
                            owner = (Org) result.institution
                ...
                if(subIdentifier) {
                    //fetch possible identifier namespaces
                    List<Subscription> subMatches = Subscription.executeQuery("select s from Subscription s where s.id = :idCandidate or s.globalUID = :idCandidate",[idCandidate:subIdentifier])
                    if(!subMatches)
                        mappingErrorBag.noValidSubscription = subIdentifier
                    else if(subMatches.size() > 1)
                        mappingErrorBag.multipleSubError = subMatches.collect { sub -> sub.dropdownNamingConvention(result.institution) }
                    else if(subMatches.size() == 1) {
                ...
                */
                ...
                if(subPkgIdentifier) {
                    if(subscription == null)
                        mappingErrorBag.packageWithoutSubscription = subPkgIdentifier
                    else {
                        List<Package> pkgMatches = Package.executeQuery("select idOcc.pkg from IdentifierOccurrence idOcc join idOcc.identifier id where idOcc.pkg.id = :idCandidate or idOcc.pkg.globalUID = :idCandidate or (id.value = :idCandidate and id.ns = :isil)",[idCandidate:subPkgIdentifier,isil:namespaces.isil])
                        if(!pkgMatches)
                ...
                */
                ...
                if(ieIdentifier) {
                    if(subscription == null || subPkg == null)
                        mappingErrorBag.entitlementWithoutPackageOrSubscription = ieIdentifier
                    else {
                        ...
                        if(!TIPPMatches)
                            ...
                            if(!ieMatches)
                                ...
                                mappingErrorBag.multipleEntitlementError = ieMatches.collect { entMatch -> "${entMatch.subscription.dropdownNamingConvention(result.institution)} - ${entMatch.tipp.title.title}" }
                            else if(ieMatches.size() == 1) {
                                ie = ieMatches[0]
                ...
                if(orderNumber) {
                    List<Order> orderMatches = Order.findAllByOrderNumber(orderNumber)
                    if(!orderMatches)
                        mappingErrorBag.noValidOrder = orderNumber
                ...
                if(invoiceNumber) {
                    List<Invoice> invoiceMatches = Invoice.findAllByInvoiceNumber(invoiceNumber)
                    if(!invoiceMatches)
                        mappingErrorBag.noValidInvoice = invoiceNumber
                ...
                try {
                    ...
                    costItem.costInBillingCurrency = costInBillingCurrency
                }
                catch (NumberFormatException e) {
                    mappingErrorBag.invoiceTotalInvalid = true
                }
                //datePaid(nullable: true, blank: false) -> to date paid
                Date datePaid = parseDate(cols[colMap.datePaid],possible_date_formats)
                if(datePaid)
                ...
                try {
                    ...
                    costItem.costInLocalCurrency = costInLocalCurrency
                }
                catch (NumberFormatException e) {
                ...
                try {
                    Double currencyRate = Double.parseDouble(cols[colMap.exchangeRate])
                    costItem.currencyRate = currencyRate
                }
                catch (NumberFormatException e) {
                 ...
                 */
                String taxType = cols[colMap.taxType]
                String taxRate = cols[colMap.taxRate]
                if(!taxType || !taxRate)
                    ...
                    if(taxRate == 7)
                        ...
                        taxKey = CostItem.TAX_TYPES.valueOf(taxType)
                    }
                    if(taxKey)
                ...
                Date invoiceDate = parseDate(cols[colMap.invoiceDate],possible_date_formats)
                if(invoiceDate)
                ...
                Date startDate = parseDate(cols[colMap.dateFrom],possible_date_formats)
                if(startDate)
                    costItem.startDate = startDate
                //endDate(nullable: true, blank: false) -> to date to
                Date endDate = parseDate(cols[colMap.dateTo],possible_date_formats)
                if(endDate)
            ...
            redirect action: 'postProcessingFinanceImport', candidates: candidates
        }
        else {
    ...
    def postProcessingFinanceImport() {
        def result = setResultGenerics()
        result
    }
