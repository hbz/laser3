    ...
    <tr>
      ...
      <td>
        <g:each in="${linkedPlatform.linkedSubs}" var="linkedSub">
          <g:link controller="Subscription" action="show" id="${linkedSub.id}">${linkedSub.name} ${(linkedSub.status != de.laser.helper.RDStore.SUBSCRIPTION_CURRENT) ? '('+ RefdataValue.getByValueAndCategory(linkedSub.status.value, RDConstants.SUBSCRIPTION_STATUS).getI10n('value') +')': ''}</g:link><br/>
        </g:each>
      </td>
      <g:if test="${ (accessService.checkPermAffiliation('ORG_BASIC_MEMBER','INST_EDITOR') && inContextOrg ) || (accessService.checkPermAffiliation('ORG_CONSORTIUM','INST_EDITOR'))}">
