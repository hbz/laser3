...
<th>
    ...
    <th class="center aligned">${message(code: 'default.copy.label')}/<br>${message(code: 'default.replace.label')}</th>
...
<g:each in="${group}" var="prop">
    ...
    <td>
        ...
        <td>
            <g:if test="${propKey.multipleOccurrence}">
                <div class="ui checkbox la-toggle-radio la-append"><input type="radio" name="subscription.takeProperty" value="${COPY}" checked/></div>
            </g:if>
            <g:else>
                <div class="ui checkbox la-toggle-radio la-replace"><input type="radio" name="subscription.takeProperty" value="${REPLACE}" checked /></div>
            </g:else>
        </td>
