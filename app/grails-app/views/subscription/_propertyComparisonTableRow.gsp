    ...
    <td>
        ...
        <td>
            <g:if test="${propValues.containsKey(sourceSubscription)}">
                    <g:each var="propValue" in="${propValues.get(sourceSubscription)}">
        ...
        <td><div class="ui checkbox la-toggle-radio la-append"><input type="radio" name="subscription.takeProperty" value="${COPY}" /></div></td>
        <td><div class="ui checkbox la-toggle-radio la-replace"><input type="radio" name="subscription.takeProperty" value="${REPLACE}" checked /></div></td>
        <td><div class="ui checkbox la-toggle-radio la-noChange"><input type="radio" name="subscription.takeProperty" value="${DO_NOTHING}" /></div></td>
        %{--TARGET-SUBSCRIPTION--}%
        <td>
            %{--<g:set var="propValues" value="${prop.getValue()}" />--}%
            ...
            <g:elseif test="${propValues.containsKey(targetSubscription)}">
                <g:each var="propValue" in="${propValues.get(targetSubscription)}">
                    %{--<g:if test="${propValue.type.multipleOccurrence && propValues.get(targetSubscription).size() > 1}">--}%
