                    ...
                    <td class="x">
                        <g:if test="${editable && accessService.checkPermAffiliationX("ORG_BASIC,ORG_CONSORTIUM","INST_EDITOR","ROLE_ADMIN")}">
                            ...
                            <g:elseif test="${surveyInfo?.surveyConfigs?.subscription?.id.contains(s.id)}">
                                <g:link class="ui icon negative button" data-tooltip="${message(code:'survey.toggleSurveySub.remove.label')}" controller="survey" action="toggleSurveySub" params="${params+[direction:'remove', sub:s.id]}">
