...
<html>
  ...
  <g:render template="public_navbar" contextPath="/templates" model="['active': 'publicExport']"/>
    ...
    <div>
      <h1 class="ui left aligned icon header"><semui:headerIcon />${license.getLicensee()?.name} ${license.type?.value} License : <semui:xEditable owner="${license}" field="reference" id="reference"/></h1>
    </div>
