...
<div class="column wide sixteen">
    ...
    <table class="ui celled la-table table">
        ...
        <%
            int affiCount = 0
            List<Org> comboOrgs = Org.executeQuery(
                    'select c.fromOrg from Combo c where c.toOrg = :org', [org: contextService.getOrg()]
            )
        %>
        <g:each in="${userInstance.affiliations}" var="aff">
            <g:if test="${tmplProfile || (editor.hasRole('ROLE_ADMIN') || (aff.org == contextService.getOrg()) || (aff.org in comboOrgs))}">
                <% affiCount++ %>
                <tr>
                    <td>
                    ...
                    <td class="x">
                        ...
                        <g:if test="${tmplUserEdit}">
                            <g:if test="${editor.hasRole('ROLE_ADMIN') || (aff.org == contextService.getOrg())|| (aff.org in comboOrgs)}">
