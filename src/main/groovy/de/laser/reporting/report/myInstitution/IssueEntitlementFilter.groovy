        ...
        switch (filterSource) {
            case 'all-ie':
                queryParams.issueEntitlementIdList = IssueEntitlement.executeQuery(
                        'select distinct(ie.id) from IssueEntitlement ie', [max: 10000] // TODO: TEST ONLY RESTRICTION
                )
                break
            case 'my-ie':
                ...
                queryParams.issueEntitlementIdList = IssueEntitlement.executeQuery(
                        ...
                        [subscriptionIdList: subIdList], [max: 10000] // TODO: TEST ONLY RESTRICTION
                )
                break
        }
...
//                        )
                    ...
                    else if (p == BaseConfig.CUSTOM_IMPL_KEY_IE_PLATFORM) {
//                        queryParts.add('TitleInstancePackagePlatform tipp')
//                        whereParts.add('ie.tipp = tipp and tipp.' + p + '.id = :p' + (++pCount)) // status
                        queryParts.add('TitleInstancePackagePlatform tipp')
                        queryParts.add('Package pkg')
                        whereParts.add('ie.tipp = tipp and tipp.pkg = pkg and pkg.' + p + '.id = :p' + (++pCount)) // status
                        //whereParts.add( 'ie.' + p + '.id in (:p' + (++pCount) + ')')
                        queryParams.put('p' + pCount, params.long(key) )
                        ...
                        queryParts.add('TitleInstancePackagePlatform tipp')
                        queryParts.add('Platform platform')  // status !!
                        whereParts.add('ie.tipp = tipp and tipp.platform = platform')
                        whereParts.add('platform.' + p + '.id = :p' + (++pCount))  // status
                        //whereParts.add( 'ie.' + p + '.id in (:p' + (++pCount) + ')')
                        queryParams.put('p' + pCount, params.long(key) )
        ...
        while (tmp) {
            queryParams.issueEntitlementIdList = tmp.take(20000)
            tmp = tmp.drop(20000) as List<Long>
            List<Long> tmpIdList = IssueEntitlement.executeQuery( query, queryParams )
            //println tmpIdList
            idList.addAll(tmpIdList)
        }
        filterResult.data.put( BaseConfig.KEY_ISSUEENTITLEMENT + 'IdList', idList.sort().toList())


        BaseConfig.getCurrentConfig( BaseConfig.KEY_ISSUEENTITLEMENT ).keySet().each{ pk ->
            if (pk != 'base') {
                if (pk == 'nominalPlatform') {
