...
class ApiCollectionReader {
    ...
    static Collection<Object> getIssueEntitlementCollectionWithSQL(SubscriptionPackage subPkg, ignoreRelation, Org context, Sql sql){
        ...
        */
        ...
        List<GroovyRowResult> priceItemIds = sql.rows("select pi_tipp_fk from price_item join title_instance_package_platform on pi_tipp_fk = tipp_id join issue_entitlement on ie_tipp_fk = tipp_id where ie_subscription_fk = :subId group by pi_tipp_fk", subParams),
        priceItemRows = []
        Array tippIds = sql.getDataSource().getConnection().createArrayOf('bigint', priceItemIds['pi_tipp_fk'].toArray() as Object[])
        for(int i = 0; i < ieCount; i += 5000) {
            priceItemRows.addAll(sql.rows("select pi_tipp_fk, json_agg(json_build_object('listCurrency', (select rdv_value from refdata_value where rdv_id = pi_list_currency_rv_fk), 'listPrice', pi_list_price, 'localCurrency', (select rdv_value from refdata_value where rdv_id = pi_local_currency_rv_fk), 'localPrice', pi_local_price)) as price_items from price_item where pi_tipp_fk = any(:tippIds) group by pi_tipp_fk limit 5000 offset "+i, [tippIds: tippIds]))
        }
