...
class SurveyController {
    ...
    Map<String, Object> currentSurveysConsortia() {
        ...
        result.editable = accessService.is_ROLE_ADMIN_or_INST_EDITOR_with_PERMS( CustomerTypeService.ORG_CONSORTIUM_PRO )
    ...
    @Secured(closure = {
        ctx.accessService.is_ROLE_ADMIN_or_INST_EDITOR_with_PERMS( CustomerTypeService.ORG_CONSORTIUM_PRO )
    })
    ...
    @Secured(closure = {
     ...
     Map<String,Object> allSurveyProperties() {
        Map<String,Object> result = surveyControllerService.getResultGenericsAndCheckAccess(params)
        if (!accessService.is_ROLE_ADMIN_or_INST_USER_with_PERMS( CustomerTypeService.ORG_CONSORTIUM_PRO )) {
            response.sendError(HttpStatus.SC_FORBIDDEN); return
        }
     ...
     */
    @DebugInfo(test = 'is_ROLE_ADMIN_or_hasAffiliation("INST_EDITOR")', wtc = DebugInfo.NOT_TRANSACTIONAL)
    ...
    @Secured(closure = {
     ...
     Map<String,Object> renewSubscriptionConsortiaWithSurvey() {
        ...
        Map<String,Object> result = surveyControllerService.getResultGenericsAndCheckAccess(params)
        result.institution = contextService.getOrg()
        if (!(result || accessService.checkCtxPerm(CustomerTypeService.ORG_CONSORTIUM_PRO))) {
     ...
     * @return a redirect to the referer
     */
    @DebugInfo(test = 'is_ROLE_ADMIN_or_hasAffiliation("INST_EDITOR")', wtc = DebugInfo.IN_BETWEEN)
    @Secured(closure = { ctx.contextService.getUser()?.is_ROLE_ADMIN_or_hasAffiliation("INST_EDITOR") })
