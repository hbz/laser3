...
//        result.putAll(gokbService.doQuery(result, params.clone(), esQuery))
    ...
    def show() {
        ...
        result.editable = accessService.checkPermAffiliationX('ORG_MEMBER_BASIC,ORG_CONSORTIUM','INST_EDITOR','ROLE_ADMIN')
     ...
     */
    @DebugInfo(perm="ORG_MEMBER_BASIC,ORG_CONSORTIUM", affil="INST_EDITOR", ctrlService = DebugInfo.WITH_TRANSACTION)
    @Secured(closure = {
        ctx.accessService.checkPermAffiliation("ORG_MEMBER_BASIC,ORG_CONSORTIUM", "INST_EDITOR")
    })
    def addDerivation() {
