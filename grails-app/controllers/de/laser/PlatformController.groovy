...
//        result.putAll(gokbService.doQuery(result, params.clone(), esQuery))
    ...
    def show() {
        ...
        result.editable = accessService.checkPermAffiliationX('ORG_BASIC,ORG_CONSORTIUM_BASIC','INST_EDITOR','ROLE_ADMIN')
     ...
     */
    @DebugInfo(perm="ORG_BASIC,ORG_CONSORTIUM_BASIC", affil="INST_EDITOR", ctrlService = DebugInfo.WITH_TRANSACTION)
    @Secured(closure = {
        ctx.accessService.checkPermAffiliation("ORG_BASIC,ORG_CONSORTIUM_BASIC", "INST_EDITOR")
    })
    def addDerivation() {
