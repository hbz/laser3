...
class OrganisationController  {
    ...
    @Secured(closure = {
        ctx.accessService.ctxInstAdmCheckPerm_or_ROLEADMIN( "FAKE,ORG_INST_BASIC,ORG_CONSORTIUM_BASIC" )
    })
    @Check404(domain=Org)
    def settings() {
        ...
        Boolean hasAccess = (result.inContextOrg && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.orgInstance, 'INST_ADM')) ||
                (isComboRelated && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.institution, 'INST_ADM')) ||
...
//            }
    ...
    def ids() {
        ...
        if(!result.orgInstance.gokbId && (result.orgInstance.sector == RDStore.O_SECTOR_PUBLISHER || RDStore.OT_PROVIDER.id in result.allOrgTypeIds)) {
            result.editable_identifier = userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.orgInstance, 'INST_EDITOR') ||
            ...
            if (accessService.ctxPerm(CustomerTypeService.ORG_CONSORTIUM_BASIC)) {
                ...
                if(consortia.size() == 1 && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.institution, 'INST_EDITOR'))
                    result.editable_identifier = true
            }
            else
                result.editable_identifier = userService.checkMinUserOrgRole_and_CtxOrg_or_ROLEADMIN(result.user, result.orgInstance, 'INST_EDITOR')
        }
        ...
        result.hasAccessToCustomeridentifier = ((inContextOrg && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.institution, 'INST_USER')) ||
                (isComboRelated && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.institution, 'INST_USER')) ||
        ...
        if (result.hasAccessToCustomeridentifier) {
            ...
            result.editable_customeridentifier = (inContextOrg && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.institution, 'INST_EDITOR')) ||
                    (isComboRelated && userService.checkMinUserOrgRole_and_CtxOrg(result.user, result.institution, 'INST_EDITOR')) ||
                    SpringSecurityUtils.ifAnyGranted('ROLE_ADMIN')
    ...
    @Secured(closure = {
        ctx.accessService.ctxInstEditorCheckPerm_or_ROLEADMIN( "FAKE,ORG_INST_BASIC,ORG_CONSORTIUM_BASIC" )
    })
    ...
    boolean checkIsEditable(User user, Org org) {
        ...
        switch(params.action){
            ...
            case 'delete':
                ...
                break
            case 'properties':
                isEditable = userService.checkMinUserOrgRole_and_CtxOrg_or_ROLEADMIN(user, Org.get(params.id), 'INST_EDITOR')
                break
            case 'users':
                isEditable = userService.checkMinUserOrgRole_and_CtxOrg_or_ROLEADMIN(user, Org.get(params.id), 'INST_ADM')
                break
            case [ 'addOrgType', 'deleteOrgType' ]:
                isEditable = userService.checkMinUserOrgRole_and_CtxOrg_or_ROLEADMIN(user, Org.get(params.org), 'INST_ADM')
                break
            ...
            case [ 'show', 'ids', 'addSubjectGroup', 'deleteSubjectGroup', 'readerNumber', 'accessPoints', 'addressbook' ]:
                    ...
                    switch (contextOrg.getCustomerType()){
                        case [ CustomerTypeService.ORG_INST_BASIC, CustomerTypeService.ORG_INST_PRO ] :
                            switch (orgInstance.getCustomerType()){
                                ...
                                default:                                        isEditable = userIsYoda; break
                            }
                            break
                ...
                break
            default:
                isEditable = userService.checkMinUserOrgRole_and_CtxOrg_or_ROLEADMIN(user, org,'INST_EDITOR')
        }
        isEditable
    }
}
