...
class UserController {
     ...
     */
    @DebugInfo(test = 'hasMinRole("ROLE_ADMIN") || hasAffiliation("INST_ADM")')
    @Secured(closure = {
        ctx.contextService.getUser()?.hasMinRole('ROLE_ADMIN') || ctx.contextService.getUser()?.hasAffiliation("INST_ADM")
    })
    @Check404()
    def delete() {
     ...
     */
    @DebugInfo(test = 'hasMinRole("ROLE_ADMIN") || hasAffiliation("INST_ADM")', wtc = DebugInfo.WITH_TRANSACTION)
    @Secured(closure = {
        ctx.contextService.getUser()?.hasMinRole('ROLE_ADMIN') || ctx.contextService.getUser()?.hasAffiliation("INST_ADM")
    })
    def newPassword() {
