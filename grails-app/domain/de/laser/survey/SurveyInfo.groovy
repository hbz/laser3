...
class SurveyInfo {
     ...
     */
    boolean isEditable() {
        if(BeanStore.getAccessService().checkConstraint_INST_EDITOR_PERM_X_RA( CustomerTypeService.ORG_CONSORTIUM_PRO ) && this.owner?.id == BeanStore.getContextService().getOrg().id)
        {
            return true
        }
