                    ...
                    currentSubsIds.each { Long id ->
                        ...
                        if ((subscription._getCalculatedType() == CalculatedType.TYPE_LOCAL) && subscription.isAllowToAutomaticRenewAnnually() && !subscription._getCalculatedSuccessor()) {
                            ...
                            if (copySub.save()) {
                                ...
                                Doc docContent = new Doc(contentType: Doc.CONTENT_TYPE_STRING, content: 'This subscription is a copy of the previous subscription. It was created automatically by the system because the flag "Automatic renew annually" was set in the previous subscription.', title: 'Automatic renew annually', type: RefdataValue.getByValueAndCategory('Note', RDConstants.DOCUMENT_TYPE), owner: org, user: null)
                                if(docContent.save()) {
