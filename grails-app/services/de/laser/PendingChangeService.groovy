...
class PendingChangeService extends AbstractLockableService {
    ...
    Map<String, Integer> getCountsForPackages(Map<SubscriptionPackage, Map<String, RefdataValue>> pkgList) {
        ...
        pkgList.each { SubscriptionPackage sp, Map<String, RefdataValue> settings ->
            ...
            List acceptedTitleCounts = IssueEntitlement.executeQuery("select count(*), 'pendingChange.message_TP04' from IssueEntitlement ie join ie.tipp tipp where tipp.pkg = :pkg and ie.subscription = :sub and ie.status = :removed", [sub: sp.subscription, pkg: sp.pkg, removed: RDStore.TIPP_STATUS_REMOVED])
