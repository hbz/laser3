            ...
            if(subscribedPlatforms && refSubs) {
                ...
                if(count5check.size() == 0) {
                    ...
                    if(params.tabStat == 'total')
                        c4usages.addAll(Counter4Report.executeQuery('select new map(title as title, r.metricType as metricType, sum(r.reportCount) as reportCount) from Counter4Report r left join r.title title where r.reportInstitution = :customer and r.platform in (:platforms) and (r.title.id in (select ie.tipp.id from IssueEntitlement ie where ie.subscription in (:refSubs) and ie.acceptStatus = :acceptStatus and ie.status = :current and ie.tipp.status = :current) or r.title is null)'+filter+dateRange+' group by title, r.metricType order by reportCount desc, title.name asc', queryParams, [max: result.max, offset: result.offset]))
                    else
                    ...
                    if(params.tabStat == 'total')
                        c5usages.addAll(Counter4Report.executeQuery('select new map(title as title, r.metricType as metricType, sum(r.reportCount) as reportCount) from Counter5Report r left join r.title title where r.reportInstitution = :customer and r.platform in (:platforms) and (r.title.id in (select ie.tipp.id from IssueEntitlement ie where ie.subscription in (:refSubs) and ie.acceptStatus = :acceptStatus and ie.status = :current and ie.tipp.status = :current) or r.title is null)'+filter+dateRange+' group by title, r.metricType order by reportCount desc, title.name asc', queryParams, [max: result.max, offset: result.offset]))
                    else
