...
/*            if (result.surveyConfig.pickAndChoosePerpetualAccess) {
    ...
    Map<String,Object> setCopyResultGenerics(GrailsParameterMap params) {
        ...
        if (!result.editable) {
            ...
            if(params.copyMyElements == true) {
                if(contextService.hasAffiliation(CustomerTypeService.ORG_INST_PRO, 'INST_EDITOR'))
                    result
            }
            else null
        }
    ...
    Map<String, Object> getResultGenericsAndCheckAccess(GrailsParameterMap params, String checkOption) {
        ...
        if (result.subscription) {
            ...
            if (checkOption in [AccessService.CHECK_EDIT, AccessService.CHECK_VIEW_AND_EDIT]) {
                result.editable = contextService.hasAffiliation(CustomerTypeService.PERMS_INST_PRO_CONSORTIUM_BASIC, 'INST_EDITOR')
            }
        }
