    ...
    private List buildIssueEntitlementRows(Set<Long> ieIDs, Map<String, Map> selectedFields, FORMAT formatEnum){
        ...
        selectedFields.eachWithIndex { String fieldKey, Map mapSelectedFields, int i ->
            ...
            if(fieldKey.startsWith('issueEntitlementIdentifiers.')) {
                ...
                queryCols << "create_cell('${format}', (select string_agg(trim(${escapeService.getFinancialOutputQuery('pi_list_price')}),';') from price_item where pi_tipp_fk = tipp_id and pi_list_currency_rv_fk = :euro), null) as listPriceEUR"
                queryArgs.euro = RDStore.CURRENCY_EUR.id
            }
            else if (fieldKey.contains('listPriceGBP')) {
                queryCols << "create_cell('${format}', (select string_agg(trim(${escapeService.getFinancialOutputQuery('pi_list_price')}),';') from price_item where pi_tipp_fk = tipp_id and pi_list_currency_rv_fk = :gbp), null) as listPriceGBP"
                queryArgs.gbp = RDStore.CURRENCY_GBP.id
            }
            else if (fieldKey.contains('listPriceUSD')) {
                queryCols << "create_cell('${format}', (select string_agg(trim(${escapeService.getFinancialOutputQuery('pi_list_price')}),';') from price_item where pi_tipp_fk = tipp_id and pi_list_currency_rv_fk = :usd), null) as listPriceUSD"
                queryArgs.usd = RDStore.CURRENCY_USD.id
            }
            else if (fieldKey.contains('localPriceEUR')) {
                queryCols << "create_cell('${format}', (select string_agg(trim(${escapeService.getFinancialOutputQuery('pi_local_price')}),';') from price_item where pi_ie_fk = ie_id and pi_local_currency_rv_fk = :leuro), null) as localPriceEUR"
                queryArgs.leuro = RDStore.CURRENCY_EUR.id
            }
            else if (fieldKey.contains('localPriceGBP')) {
                queryCols << "create_cell('${format}', (select string_agg(trim(${escapeService.getFinancialOutputQuery('pi_local_price')}),';') from price_item where pi_ie_fk = ie_id and pi_local_currency_rv_fk = :lgbp), null) as localPriceGBP"
                queryArgs.lgbp = RDStore.CURRENCY_GBP.id
            }
            else if (fieldKey.contains('localPriceUSD')) {
                queryCols << "create_cell('${format}', (select string_agg(trim(${escapeService.getFinancialOutputQuery('pi_local_price')}),';') from price_item where pi_ie_fk = ie_id and pi_local_currency_rv_fk = :lusd), null) as localPriceUSD"
