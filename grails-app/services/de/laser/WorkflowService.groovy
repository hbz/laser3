...
//        println wfList.collect{ it.id + '_' + it.getInfo().lastUpdated }.take(5)
    ...
    boolean hasUserPerm_read() {
        ...
        if (ctxOrg.isCustomerType_Consortium_Pro() && contextService.getUser().hasOrgAffiliation_or_ROLEADMIN(ctxOrg, 'INST_USER')) {
            return true
        }
        false
    }
    ...
    boolean hasUserPerm_edit() {
        Org ctxOrg = contextService.getOrg()
        if (ctxOrg.isCustomerType_Consortium_Pro() && contextService.getUser().hasOrgAffiliation_or_ROLEADMIN(ctxOrg, 'INST_EDITOR')) {
            return true
        }
        false
    }
