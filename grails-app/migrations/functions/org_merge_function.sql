...
BEGIN
	...
	for oprow1 in select op_string_value, op_type_fk from org_property where op_owner_fk = new_key
		loop
	...
	for oprow2 in select op_date_value, op_type_fk from org_property where op_owner_fk = new_key
		loop
	...
	for oprow3 in select op_dec_value, op_type_fk from org_property where op_owner_fk = new_key
		loop
		delete from org_property where op_dec_value = oprow3.op_dec_value and op_type_fk = oprow3.op_type_fk and op_owner_fk = old_key;
		end loop;
	for oprow4 in select op_int_value, op_type_fk from org_property where op_owner_fk = new_key
		loop
		delete from org_property where op_int_value = oprow4.op_int_value and op_type_fk = oprow4.op_type_fk and op_owner_fk = old_key;
		end loop;
	for oprow5 in select op_ref_value_rv_fk, op_type_fk from org_property where op_owner_fk = new_key
		loop
	...
	for oprow6 in select op_url_value, op_type_fk from org_property where op_owner_fk = new_key
		loop
	...
	if(orcount > 0) then
		for orrow1 in select or_id, or_lic_fk from org_role where or_org_fk = new_key
			loop
		...
		for orrow2 in select or_id, or_sub_fk from org_role where or_org_fk = new_key
			loop
		...
		for orrow3 in select or_tipp_fk from org_role where or_org_fk = new_key
			loop
			delete from org_role where or_tipp_fk = orrow3.or_tipp_fk and or_org_fk = old_key;
			end loop;
		for orrow4 in select or_pkg_fk from org_role where or_org_fk = new_key
			loop
	...
	for osrow1 in select os_key_enum, os_string_value from org_setting where os_org_fk = new_key
		loop
	...
	for osrow2 in select os_key_enum, os_rv_fk from org_setting where os_org_fk = new_key
		loop
		delete from org_setting where os_rv_fk = osrow2.os_rv_fk and os_key_enum = osrow2.os_key_enum and os_org_fk = old_key;
		end loop;
	for osrow3 in select os_key_enum, os_role_fk from org_setting where os_org_fk = new_key
		loop
	...
	for osrow4 in select os_key_enum from org_setting where os_org_fk = new_key and os_string_value is null and os_rv_fk is null and os_role_fk is null
		loop
		delete from org_setting where os_key_enum = osrow4.os_key_enum and os_org_fk = old_key;
		end loop;
	...
	delete from person_role WHERE pr_org_fk = old_key and pr_prs_fk in (select pr_prs_fk from person_role where pr_org_fk = new_key);
	update person_role set pr_org_fk = new_key where pr_org_fk = old_key;
	delete from person WHERE prs_tenant_fk = old_key and prs_last_name in (select prs_last_name from person where prs_tenant_fk = new_key);
