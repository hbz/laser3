...
databaseChangeLog = {
    ...
    changeSet(author: "djebeniani (hand-coded)", id: "1684995746509-4") {
        grailsChange {
            change {
                sql.executeUpdate("delete from refdata_value join refdata_category on rdv_owner = rdc_id where rdc_description = 'tipp.status' and rdv_value = 'Transferred'))")
            }
            rollback {}
        }
    }
    ...
    changeSet(author: "djebeniani (hand-coded)", id: "1684995746509-5") {
        grailsChange {
            change {
                sql.executeUpdate("delete from refdata_value join refdata_category on rdv_owner = rdc_id where rdc_description = 'tipp.status' and rdv_value = 'Unknown'))")
            }
            rollback {}
        }
    }
