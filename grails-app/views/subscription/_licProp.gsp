    ...
    <g:if test="${derivedPropDefGroups.global || derivedPropDefGroups.local || derivedPropDefGroups.member || derivedPropDefGroups.orphanedProperties}">
        ...
        <%-- custom properties --%>
            ...
            <g:if test="${derivedPropDefGroups.orphanedProperties}">
                <g:set var="filteredOrphanedProperties" value="${derivedPropDefGroups.orphanedProperties.findAll { prop -> (prop.tenant?.id == contextService.org.id || !prop.tenant) || prop.isPublic || (prop.hasProperty('instanceOf') && prop.instanceOf && AuditConfig.getConfig(prop.instanceOf))}}"/>
            </g:if>
