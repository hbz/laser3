...
<g:if test="${success}">
    <ui:msg class="success" icon="${Icons.UI.SUCCESS}" message="subscription.details.addEntitlements.matchingSuccessful" args="[processCount, processRows, countSelectTipps, countNotSelectTipps, g.createLink(controller: 'subscription', action: 'index', params: [id: subscription.id])]"/>
</g:if>
...
<g:elseif test="${errMess}">
    <ui:msg icon="${Icons.UI.ERROR}" class="error" message="${errMess}" args="[errorCount]"/>
