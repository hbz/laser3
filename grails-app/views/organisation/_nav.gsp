...
<ui:subNav actionName="${actionName}">
    ...
    <g:if test="${inContextOrg}">
        <ui:subNavItem controller="organisation" action="myPublicContacts" params="${[id: institution.id]}" message="menu.institutions.publicContacts" />
    </g:if>
    <g:elseif test="${(customerTypeService.isConsortium( contextCustomerType ) && !isProviderOrAgency)}">
        <ui:subNavItem controller="organisation" action="myPublicContacts" params="${breadcrumbParams}" message="menu.institutions.publicContacts" />
    </g:elseif>
    <g:else>
        <g:if test="${!isProviderOrAgency}">
            <ui:subNavItem controller="organisation" action="myPublicContacts" message="menu.institutions.publicContacts" disabled="true" />
        </g:if>
    </g:else>
    <g:if test="${!isProviderOrAgency}">
