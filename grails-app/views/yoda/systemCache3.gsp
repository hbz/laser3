<%@ page import="de.laser.ui.Btn; de.laser.Cache3Service; de.laser.cache.Ehcache3Wrapper; groovy.json.JsonBuilder; de.laser.utils.DateUtils" %>
...
<%
    ...
    Ehcache3Wrapper t1 = cache3Service.getTTL300Cache('test')
    t1.put('aaa', new Object())

    List ehCaches = [
            cache3Service.getCacheNames().findAll { it -> !it.startsWith('de.laser.')},
            cache3Service.getCacheNames().findAll { it -> it.startsWith('de.laser.')}
    ]
...
<g:each in="${ehCaches}" var="ehCache">
    ...
    <g:each in="${ehCache.toSorted()}" var="cacheName">
        <g:set var="cache" value="${ehcacheManager.getCache(cacheName, String.class, Object.class)}" />
...
%{--                <g:if test="${cache3Service.getDiskStorePath(cache.getCacheManager())}">--}%
...
%{--                )--}%
        ...
        <div class="ui segments">
            ...
            <div class="ui segment">
                ...
                <g:if test="${false /*cache.getKeysWithExpiryCheck().size() > 0*/}">
