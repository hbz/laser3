...
<g:if test="${contextService.getOrg().isCustomerType_Pro()}">
    ...
    <ui:modal id="modalRttpCreateTask" message="task.create.reportTitleToProvider">
        ...
        <g:form id="${preID}_form" class="ui form" url="[controller: 'task', action: 'createTask']" method="post">
            ...
            <g:hiddenField id="${preID}_tipp" name="tipp" value="//tipp.id//"/>
        ...
        <laser:script file="${this.getGroovyPageFileName()}">
            ...
            JSPC.callbacks.modal.onShow.modalRttpCreateTask = function (trigger) {
                let preID = '#' + $('#modalRttpCreateTask form input[name=preID]').val()
                console.log ( 'modalRttpCreateTask / preID: ' + preID )
                console.log ( trigger )
