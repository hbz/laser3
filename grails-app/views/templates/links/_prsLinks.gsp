...
<table class="ui celled la-js-responsive-table la-table table">
    ...
    <g:each in="${visiblePrsLinks}" var="pr">
        <tr>
            <g:if test="${pr.org}">
                <td>
                    <g:if test="${! pr.prs.isPublic}"><i class="${Icon.ACP_PRIVATE}"></i> </g:if>
                    ${pr.prs}
                </td>
                <g:if test="${tmplShowFunction}">
                    <td>
                ...
                <td>
                    <g:if test="${editable}">
                        <g:link controller="addressbook" action="deletePersonRole" id="${pr.id}" class="${Btn.MODERN.NEGATIVE_CONFIRM}"
                                role="button"
