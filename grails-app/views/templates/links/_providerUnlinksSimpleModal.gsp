    ...
    <laser:script file="${this.getGroovyPageFileName()}">
        ...
        $('#${tmplModalID}_orgSearch').on('input', function() {
            ...
            searchTimer = setTimeout(function() {
                $.ajax({
                    <g:if test="${parent}">
                        url: "<g:createLink controller="ajaxHtml" action="lookupProviders"/>?tableView=true&unlink=true&subscription=${parent}&query="+searchVal,
                    </g:if>
                    <g:else>
                        url: "<g:createLink controller="ajaxHtml" action="lookupProviders"/>?tableView=true&unlink=true&query="+searchVal,
                    </g:else>
