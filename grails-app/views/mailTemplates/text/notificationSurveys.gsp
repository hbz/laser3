<%@ page import="de.laser.config.ConfigMapper; de.laser.storage.RDConstants; de.laser.RefdataValue; de.laser.storage.RDStore; de.laser.UserSetting;" %><laser:serviceInjection /><g:set var="grailsApplication" bean="grailsApplication" />
<g:message code="email.survey.title" locale="${language}"/>
<g:each in="${surveys}" var="survey"><g:set var="surveyUrl" value="${"/myInstitution/surveyInfos/${survey.id}?surveyConfigID=${survey.surveyConfigs[0].id}"}"/><g:set var="renewalSurvey" value="${survey.type == RDStore.SURVEY_TYPE_RENEWAL}"/><g:set var="linkToSurvey" value="${ConfigMapper.getConfig('grails.serverURL', String)+raw(surveyUrl)}"/>
...
<g:message code="email.survey.general.help" locale="${language}" args="[surveys[0].owner.name]"/>

${message(code: 'email.text.end', locale: language)}
${message(code: 'email.survey.owner', locale: language)}
${surveys[0].owner.name}
