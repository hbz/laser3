...
<g:if test="${result}">
    ...
    option = {
       title: {
           text: 'Bundesländer aller Teilnehmer',
           left: 'center'
       },
       tooltip: {
           trigger: 'item'
               ...
               data: [
                <%
                    List dummy = Org.executeQuery(
                            'select c.value_de, count(*) from Org o join o.region c where o.id in :memberIdList group by c.value_de',
                            [memberIdList: result.memberIdList]
                    )
                    dummy.each{ it ->
