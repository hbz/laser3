...
<g:if test="${filterResult}">
    ...
    <g:if test="${validFilterResult}">
        <div class="ui message ${filterResult.get(ElasticSearchHelper.ELASTICSEARCH_IS_NOT_REACHABLE) ? 'warning' : 'success'}">
            ...
            <g:if test="${filterResult.get(ElasticSearchHelper.ELASTICSEARCH_IS_NOT_REACHABLE)}">
                <p><i class="${Icons.SYM.WARNING} large"></i> ${message(code:'reporting.filter.result.x.elasticSearchIsNotReachable')}</p>
    ...
    <g:else>
        ...
        <g:if test="${filterResult.get(ElasticSearchHelper.ELASTICSEARCH_IS_NOT_REACHABLE)}">
            <ui:msg class="error" icon="${Icons.SYM.ERROR}" message="reporting.filter.result.x.elasticSearchIsNotReachable" noClose="true" />
        </g:if>
    </g:else>
