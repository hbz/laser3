...
%{--            <a class="item" data-tab="issueEntitlement-filter-tab-help"> ? <i class="${Icons.MSG.HELP}"></i></a>--}%
        </div><!-- .menu -->
...
%{--        <g:if test="${config}">--}%
%{--            <input type="hidden" name="filter:nominalPlatform_source" value="filter-subset-nominalPlatform" />--}%
%{--        </g:if>--}%

        <g:set var="config" value="${BaseConfig.getCurrentConfig( BaseConfig.KEY_ISSUEENTITLEMENT ).package}" />
        <g:if test="${config}">
            <input type="hidden" name="filter:package_source" value="filter-subset-package" />
        </g:if>
        ...
        <g:if test="${config}">
            <input type="hidden" name="filter:subscription_source" value="filter-subset-subscription" />
        </g:if>
