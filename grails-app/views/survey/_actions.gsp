...
<g:if test="${contextService.isInstEditor(CustomerTypeService.ORG_CONSORTIUM_PRO)}">
    ...
    <g:if test="${(actionName == 'surveyCostItems' || actionName == 'surveyCostItemsPackages' || actionName == 'surveyCostItemsSubscriptions')}">
        <ui:modal id="bulkCostItemsUpload" message="menu.institutions.financeImport"
            ...
            <g:render template="/templates/genericFileImportForm" model="[processAction: 'processSurveyCostItemsBulkWithUpload', id: surveyInfo.id, surveyConfigID: surveyConfig.id, surveyPackage: (actionName == 'surveyCostItemsPackages'), surveySubscriptions: (actionName == 'surveyCostItemsSubscriptions'), hideSubmitButtons: true]"/>
            %{--
