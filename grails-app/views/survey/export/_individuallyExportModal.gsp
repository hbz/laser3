...
<ui:modal id="${modalID}" text="Excel-Export" hideSubmitButton="true">
    ...
    <g:form controller="survey" action="surveyEvaluation" refreshModal="true" id="${surveyInfo.id}"
        ...
        <laser:render template="/templates/export/individuallyExportForm" model="${[formFields: formFields, exportFileName: escapeService.escapeString(surveyConfig.getSurveyName()) + "_" + message(code:'surveyResult.label'), contactSwitch: true, csvFieldSeparator: '|', overrideFormat: [xlsx: 'XLSX', csv: 'CSV']]}"/>
