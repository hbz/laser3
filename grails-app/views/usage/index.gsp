<%@ page import="de.laser.ui.Button; de.laser.ui.Icon; de.laser.config.ConfigMapper" %>
...
<ui:filter simple="true">
    <g:form action="index" controller="usage" method="get" class="ui small form">
        <div class="three fields">
            ...
            <div class="fields">
                <div class="field">
                    <g:actionSubmit action="fetchSelection" class="${Button.PRIMARY}" value="${message(code: 'default.usage.adminPage.button.fetchSelection')}" onclick="return confirm('${message(code:'confirm.start.StatsSync')}')"/>
                </div>
                <div class="field">
                    <g:actionSubmit action="deleteSelection" class="${Button.PRIMARY}" value="${message(code: 'default.usage.adminPage.button.deleteSelection')}" onclick="return confirm('${message(code:'confirm.start.StatsDeleteSelection')}')"/>
                </div>
                <div class="field">
...
<table class="ui sortable celled la-js-responsive-table la-table table compact">
  ...
  <g:each in="${availStatsRanges}" var="asr" status="i">
    ...
    <g:if test="${statsError.size()!=0}">
      <td class="x">
        <button class="${Button.BASIC_ICON}" data-target="stats-error-content-${i}">
