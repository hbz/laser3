                                ...
                                <%
                                    ...
                                    if (dc.license) {
                                        println "License ${dc.license.id} - ${dc.license.reference}, ${dc.license.sortableReference} &nbsp;&nbsp; " +
                                                link(action: 'show', controller: 'lic', id: dc.license.id) { '<i class="' + Icons.LNK.EXTERNAL + '"></i>' }
                                    }
                                    if (dc.subscription) {
                                        println "Subscription ${dc.subscription.id} - ${dc.subscription.name} &nbsp;&nbsp; " +
                                                link(action: 'show', controller: 'subscription', id: dc.subscription.id) { '<i class="' + Icons.LNK.EXTERNAL + '"></i>' }
                                    }
                                    if (dc.org) {
                                        println "Org ${dc.org.id} - ${dc.org.name} ${dc.org.sortname} &nbsp;&nbsp;" +
                                                link(action: 'show', controller: 'org', id: dc.org.id) { '<i class="' + Icons.LNK.EXTERNAL + '"></i>' }
                                    }
                                    if (dc.link) {
                                    ...
                                    if (dc.surveyConfig) {
                                        println "SurveyConfig ${dc.surveyConfig.id} - ${dc.surveyConfig.type} ${dc.surveyConfig.header} &nbsp;&nbsp; " +
                                                link(action: 'surveyConfigDocs', controller: 'survey', id: dc.surveyConfig.surveyInfo.id, params: ['surveyConfigID': dc.surveyConfig.id]) { '<i class="' + Icons.LNK.EXTERNAL + '"></i>' }
                                    }
