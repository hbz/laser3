<laser:htmlStart message="menu.admin.mergeProviders" serviceInjection="true"/>
    ...
    <g:form controller="admin" action="mergeProviders">
        <table class="ui celled la-js-responsive-table la-table compact table">
            <thead>
                <tr>
                    <th colspan="3">
                        <label for="source">${message(code: 'default.provider.label')}</label>
                    ...
                    <th colspan="3">
                        <label for="target">${message(code: 'default.provider.label')}</label>
    ...
    <laser:script file="${this.getGroovyPageFileName()}">
        $('.org').each(function() {
            $(this).dropdown({
                apiSettings: {
                    url: '<g:createLink controller="ajaxJson" action="lookupProviders"/>?query={query}&displayWekbFlag=true',
                    cache: false
                },
                clearable: true,
                minCharacters: 0
