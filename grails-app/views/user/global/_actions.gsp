...
<g:elseif test="${actionName == 'edit'}">%{-- /user/edit --}%
    <g:if test="${SpringSecurityUtils.ifAnyGranted('ROLE_ADMIN')}">
        <ui:actionsDropdown>
            <g:link class="item" action="delete" id="${params.id}"><i class="${Icons.CMD_DELETE} icon"></i> ${message(code:'deletion.user')}</g:link>
        </ui:actionsDropdown>
    </g:if>
</g:elseif>
...
<g:elseif test="${actionName == 'editUser'}">
    <g:if test="${controllerName == 'myInstitution'}">%{-- /myInstitution/editUser --}%
        <g:if test="${contextService.isInstAdm_or_ROLEADMIN()}">
            <ui:actionsDropdown>
                <g:link class="item" action="deleteUser" params="${[uoid: params.uoid]}"><i class="${Icons.CMD_DELETE} icon"></i> ${message(code:'deletion.user')}</g:link>
            </ui:actionsDropdown>
        </g:if>
    </g:if>
    <g:elseif test="${controllerName == 'organisation'}">%{-- /organisation/editUser - TODO: isComboInstAdminOf --}%
        <g:if test="${SpringSecurityUtils.ifAnyGranted('ROLE_ADMIN')}">
            <ui:actionsDropdown>
                <g:link class="item" action="deleteUser" params="${[id:params.id, uoid: params.uoid]}"><i class="${Icons.CMD_DELETE} icon"></i> ${message(code:'deletion.user')}</g:link>
            </ui:actionsDropdown>
        </g:if>
    </g:elseif>
</g:elseif>
